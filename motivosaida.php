...
<?php $titulo_pagina = 'Motivo Saída'; ?>
<?php if (isset($_POST['add_motivo'])) {
    if ($_POST['nome'] != null) {
        $nome = limpaString($_POST['nome']);
        $sql = "INSERT INTO motivosaida (nomeMotivo) VALUES ('" . $nome . "')";
        if ($db->query($sql)) {
            $session->msg("s", "Motivo inserido com sucesso.");
            redirect('motivosaida.php', false);
        } else {
...
<?php $motivosaida = tabelaMotivoSaida(); ?>
            ...
            <div class="row">
                <div class="col-xl-12">
                    <div class="card card-default">
                        <div class="card-header clearfix">
                            <div class="header-block">
                                <p class="title"> Todos os Motivos </p>
                            </div>
                        </div>
                        <div class="card-body">
                            <table class="table table-striped table-bordered table-condensed">
                                ...
                                <tbody>
                                    <?php foreach ($motivosaida as $p) : ?>
                                        <tr>
                                            <td><?php echo $p['id']; ?></td>
                                            <td><?php echo $p['nomeMotivo']; ?></td>
                                            <td><a href="editaMotivoSaida?id=<?php echo $p['id']; ?>" class="btn btn-info"><i class="fa fa-edit" style="color: white"></i></a></td>
                                        </tr>
                                    <?php endforeach; ?>
                                </tbody>
                            </table>
