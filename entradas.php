...
<?php verificaNivelAcesso(array(1, 2, 3)); ?>
<?php $titulo_pagina = 'Entradas'; ?>
<?php $produtos = tabelaProdutos(); ?>
<?php $motivoentrada = executaSql("SELECT * FROM motivoentrada"); ?>
<?php
$pag = $_GET["p"];
$s = 30 * ($pag - 1);
$r = 30;
...
if (isset($_POST['adicionar_Entrada'])) {
	...
	$sql = "INSERT INTO entradas ( `idMotivo`, `dataEntrada`, `idProduto`, `qtdProduto`) VALUES";
	for ($i = 0; $i < $times; $i++) {
		$sql .= " ('{$_POST['motivo'][$i]}', NOW() , '{$_POST['produto'][$i]}','{$_POST['qtd'][$i]}'),";
		entradaEstoque($_POST['produto'][$i], (int)$_POST['qtd'][$i]);
	}
	$sql = substr($sql, 0, -1);
	$sql .= ";";
	global $db;
	$result = $db->query($sql);
	if ($result) {
		$session->msg('s', ' Entrada Adicionada');
		redirect('entradas.php?p=1', false);
	} else {
	...
	<div class="content">
				...
				<div class="card card-default">
					<div class="card-header bordered">
						<div class="header-block">
							<h3 class="title"> Adicionar Entrada </h3>
						</div>
					</div>
					<div class="card-body">
						<div class="row">
							...
							<div class="col-md-3">
								...
								<select class="form-control mb-2 mr-sm-2" id="motivo" required>
									...
									<?php
									foreach ($motivoentrada as $f) : ?>
										<option value="<?php echo $f['id']; ?>">
											<?php echo $f['nomeMotivo']; ?>
										</option>
						...
						<form method="post">
							...
							<button type="submit" name="adicionar_Entrada" class="btn btn-primary">Confirmar Entrada</button>
						</form>
		...
		<section class="section">
					...
					<div class="card card-default">
						...
						<div class="card-body">
							<table class="table table-striped table-bordered table-condensed">
								...
								<tbody>
									<?php $entradas = tabelaEntradas($s, $r); ?>
									<?php foreach ($entradas AS $sp) : ?>
										<tr>
											<td><?php echo $sp['idEntrada']; ?></td>
											<td><?php echo $sp['nomeProduto']; ?></td>
											<td><?php echo $sp['qtdProduto']; ?></td>
											<td><?php echo $sp['nomeMotivo']; ?></td>
											<td><?php echo date("d/m/Y", strtotime($sp['dataEntrada'])); ?></td>
											<td><a href="deletaEntrada.php?id=<?php echo $sp['idEntrada']; ?>" class="btn btn-danger"><i class="fa fa-trash"></i></a></td>
										</tr>
									<?php endforeach; ?>
								</tbody>
							</table>
			...
			<nav aria-label="Page navigation">
				<ul class="pagination">
					<?php $paginacao = contaPaginacao($r,"entradas"); ?>
					<?php if((int)$pag>1): ?>
					<li class="page-item"><a class="page-link" href="entradas.php?p=1">1</a></li>
					<li class="page-item"><a class="page-link">...</a></a></li>
					<li class="page-item"><a class="page-link" href="entradas.php?p=<?php echo (int)$pag-1;?>"><?php echo (int)$pag-1;?></a></li>
					<?php endif;?>
					<li class="page-item active"><a  class="page-link"><?php echo (int)$pag;?></a></li>
					<?php if((int)$pag<=$paginacao): ?>
					<li class="page-item"><a class="page-link" href="entradas.php?p=<?php echo (int)$pag+1;?>"><?php echo (int)$pag+1;?></a></li>
					<li class="page-item"><a class="page-link">...</a></li>
					<li class="page-item"><a class="page-link" href="entradas.php?p=<?php echo (int)$paginacao+1;?>"><?php echo (int)$paginacao+1;?></a></li>
					<?php endif;?>
				</ul>
			</nav>
			</div>
